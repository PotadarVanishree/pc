  // variables whose values are set in initMap() test
  var mapName;
  var credential;
    ...
    if (event == "mousedown") {
      ...
      if (windowEvent.shiftKey) {
        ...
        if (!fetchingTooltip && !windowEvent.shiftKey) {
          ...
          // use these values for determining the tooltip location
          x = e.getX()
          y = e.getY()
          this.fetchTip();
        }
  ...
  HeatMap.prototype.postToServer = function (urlParamString, callback) {
    ...
    if (csrfToken) {
        ...
        request.send("csrfToken=" + csrfToken);
    } else {
        request.send(".");
    }
  };
  ...
  HeatMap.prototype.fetchTip = function() {
    ttLatLong = map.tryPixelToLocation(new Microsoft.Maps.Point(ttMouseDownPosition.getX(), ttMouseDownPosition.getY()));
