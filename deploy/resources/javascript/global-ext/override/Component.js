Ext.define('Gw.override.Component', {
  ...
  onEnterShortcutActivated: function (key, evt) {
    ...
    if (this.disabled || tagName == 'TEXTAREA' || tagName == 'BUTTON' || tagName == 'A') {
      return; // Do not invoke default ENTER handler, if the target has its own action or is disabled.
    }
    // Make sure that comp that is mapped to the Enter key is on screen.
    if (gw.app.EnterShortcutId != null) {
      gw.app.handleAction(null, gw.app.EnterShortcutId);
    }
  },
  ...
  addKeyMap: function (key, scopeEl, config, comp) {
    ...
    if (mapsPerKey.length > 0) {
      ...
      oldComp = old.comp;
      //PL-32196 We need to make sure oldComp.origText exist because in some cases it is undefined (i.e., Next Button)
      if (oldComp.origText && oldComp.origText !== oldComp.text) {
        textChanged = true;
