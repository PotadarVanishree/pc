Ext.define('Gw.overrides.selection.CellModel', {
  ...
  _processCheckboxOrSimpleRadioCell: function (e) {
    ...
    if (rec) {
      ...
      if (fieldName !== '_Checkbox' &&  Ext.fly(cell).query('[class^=x-grid-checkcolumn]').length > 0) {
          gw.GridUtil.processCheckBoxCellAction(view.panel, rec, pos.columnHeader, pos.row);
  ...
  onEditorTab: function(editingPlugin, e) {
    ...
    // Copy the parsed value into the field so that hiding the editor shows the correct value. See PLWEB-4465 - Editable
    // List View Date Fields do not apply format on tab out. This may not be the ideal place to do this: When
    // fixing PLWEB-3140 - Tabbing off DateCell should not clear invalid data; we need to understand how grid keyboard
    // handling works, and determine the right order of clearing
    var activeEditor = editingPlugin.getActiveEditor();
    if (activeEditor &&
        activeEditor.field &&
        activeEditor.field.xtype === "datefield") {
      var rawValue = activeEditor.field.formatDate(activeEditor.field.getValue());
      activeEditor.field.setRawValue(rawValue);
    }
