Ext.define('Gw.override.view.Table', {
  ...
  // @SenchaUpgrade (not for ExtJS5): Only focus on the row if the browser is not IE or if the content is not horizontally
  // scrollable.
  focusCell: function(position, delay) {
    ...
    if (me.isVisible(true) && (cell = me.getCellByPosition(position))) {
      ...
      // @SenchaUpgrade: Use our center panel for the correct calculation
      dom = Ext.get('centerPanel').el.dom;
      // Another IE bug. If the gridview el has overflowed and the user is navigating down a column (either mouse or keyboard),
      // there could be a percieved 'flickering'.  This is due to that fact that IE scrolls the div back to the 0 position and
      // then we're scrolling it back to its previous position to work around this (see the #doFocus method below).
      //
      // The hack below will not focus the row if it's an IE browser and the div has overflowed.  Note this isn't a problem in
      // v5 because we focus cells, not rows.
      // See EXTJS-10414.
      if (!(Ext.isIE && dom.scrollWidth > dom.clientWidth)) {
  ...
  renderCell: function(column, record, recordIndex, rowIndex, columnIndex, out) {
    ...
    if(fieldValue.wrap) {
      classes[clsInsertPoint++] = me.wrapCls;
    }
