Ext.define('Gw.override.grid.plugin.CellEditing', {
  ...
  init: function () {
    ...
    // PLWEB-4287: Set the invalidFormat flag here if inputMask validation failed in the editor
    this.on('validateedit', function(editor, editEvt) {
      var column = editEvt.column;

      if (column.hasEditor()) {
        var record = editor.context.record,
            ed = editor.editors.getByKey(column.getItemId());

        if (ed.field.activeError) {
          record.getData()[column.dataIndex].invalidFormat = true;
        }
        else {
          record.getData()[column.dataIndex].invalidFormat = false;
        }
      }
    });
    ...
    this.on('beforeedit', function (editor, editEvt) {
      ...
      // PLWEB-5767 is a regression introduced by the ignoreGwFocusEventsWhileExitingEditor flag
      // before editing start, set ignore to true so that editing can continue
      editor.getCmp().getSelectionModel().ignoreGwFocusEventsWhileExitingEditor = true;
  ...
  _blurFocusOnNonEditableCellIfNecessary: function() {
    ...
    if (me.cellNav) {
      var grid = me.grid,
          ...
          pos = model.getCurrentPosition();
      if (pos) {
        var cell = me.getCell(pos.record, pos.columnHeader),
            dom = Ext.get(cell).query('*[tabindex]');
