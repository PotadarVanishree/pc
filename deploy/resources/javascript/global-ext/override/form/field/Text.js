Ext.define('Gw.override.form.field.Text', {
  ...
  initComponent: function () {
    ...
    if (me.gNumCols) {   // The size of the text field in characters
      // Convert size to pixels, using the width of the M character in pixels (fix for PL-31540)
      me.inputWidth = me.gNumCols * gw.app.widthOfMCharInPixels();
    }

    // for toolbar input when there is a ReflectOnBottom configured
    if (this.otherToolbarReflectId) {
      this.on("change", this.updateReflector, this);
    }
  ...
  updateReflector: function (comp) {
    gw.ext.Util.setValueWithSuspendChangeEvent(comp.otherToolbarReflectId, comp.value);
  }
});
