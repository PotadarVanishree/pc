...
Ext.define('gw.SimpleGrid', {
  ...
  aboutToEdit: null, // IMPORTANT: needed to fix PL-31352 to remember the state of an soon-to-be edited
  ...
  initComponent: function () {
    ...
    if (me.selType === 'cellmodel') {
      ...
      me.on('beforeedit', function(editor, editingInfo) {
        if (me.keepSelection && editingInfo) {
          // The editor's editing flag is not set until AFTER the current position in
          // the grid is set.  So we remember that we are about to edit the row/column
          // specified in the editing info.  We will use this later when the
          // 'selectionchange' event is fired
          me.aboutToEdit = {
            r: editingInfo.rowIdx,
            c: editingInfo.colIdx
          };
          Ext.defer(function() {
            // we defer clearing the about to edit flag after a delay to allow the
            // deselect/select selectionchange events to flow through the system
            me.aboutToEdit = null;
          }, 200);
        }
      });
      ...
      me.on('selectionchange', function (model, selection) {
        if (me.keepSelection && selection.length > 0) {
          var model = me.getSelectionModel(),
              ...
              editing = me.editingPlugin ? me.editingPlugin.editing : false;
          // Either the editor is already editing, or we know we are about to edit a cell at the
          // particular row/column.  If the cell position we are about to edit matches the current
          // selection, then we are in fact editing for the sake of remembering the state
          if (!editing && me.aboutToEdit && me.aboutToEdit.r === sel.row &&
              me.aboutToEdit.c === sel.column) {
            editing = true;
          }
          // store the row, column, current edit and selection model was editing states in the hash map
          gw.app.gridLastSelection[me.id] = {
            r: sel.row,
            c: sel.column,
            e: editing,
            we: me.selModel && me.selModel.wasEditing
          }
        } else {
          // remove key
        ...
        if (me.keepSelection && grid.selType == 'cellmodel' && sel[me.id]) {
          ...
          if (gw.app.getActiveLV() == null || gw.app.getActiveLV() == me.id) {
            var row = sel[me.id].r,
                column = sel[me.id].c,
                editing = sel[me.id].e,
                wasEditing = sel[me.id].we;
            if (model.view.getRecord(row)) {
              model.setCurrentPosition({row: row, column: column});
              // If we have an editing plugin and the selection was last editing
              // start the editor (we need a slight delay to allow render to finish cleanly)
              if (me.editingPlugin && editing) {
                var currentPos = model.getCurrentPosition();
                Ext.defer(function() {
                  if (me && me.editingPlugin && currentPos) {
                    // Make sure we have the data still available.  When tests are running it may already be gone.
                    me.editingPlugin.startEdit(currentPos.record, currentPos.columnHeader);
                  }
                }, 300);
              }
              if (wasEditing && me.selModel) {
                // If the selection model was editing and we still have a selection model then restore the value
                me.selModel.wasEditing = true;
              }
            }
          }
        }
    ...
    if (this.data && this.data.options && this.data.options.limit != undefined) {
      ...
      if (this.bbar) {
        this.bbar = Ext.apply(this.bbar, {
          store: this.store,
          gridId: this.id,
          // The paging/record count display id is <lvId>:_ListPaging.
          // This id is needed for testing and updating the LV page count. lvId is the grid's LV
          // id or the dataUrl (dominant row iterator) if the LV has a render id only (no explicit id)
          pagingId: dominantRowIteratorId + ':_ListPaging__dup_1',
          recordCountId: dominantRowIteratorId + ':_RecordCount__dup_1',
          pagingDisabled: this.data.options.pagingDisabled,
          displayInfo: this.displayInfo,
          prependButtons: true, // add other buttons before the pagination
          plugins: {ptype: 'gtbconfig'}, // default config for gw toolbar
        ...
        if (this.bbar.items) {
          this.bbar.items.push('-'); // a divider between pagination and other controls
        }
      }
    ...
    if (this.tbar && this.bbar) {
      gw.ext.Util.linkBottomAndTopToolbar(this);
    }

    // add filters to panel toolbar
    if (this.filters) {
      gw.GridUtil.addFiltersToPanel(this, this.filters, this.bbarFilters);
      delete this.filters;
    }
    ...
    //reset the LV cell focus when sorting is happening
    this.on('sortchange', function(grid) {
      this.getSelectionModel().deselect();
    });
  },
  ...
  processCellClickViaMouse: function (gridView, td, cellIndex, record, tr, rowIndex, evt) {
    ...
    var gridPanel = gridView.panel;
      ...
      if (fieldName != '_Checkbox' && t.className.indexOf(Ext.baseCSSPrefix + 'grid-checkcolumn') != -1) {
        ...
        gw.GridUtil.processCheckBoxCellAction(gridPanel, record, column, rowIndex);
