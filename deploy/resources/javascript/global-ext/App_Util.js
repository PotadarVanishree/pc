...
gw.ext.Util = function() {
   ...
   */
  function setRadioGroupValue(comp, value) {
    if (value == "" && !radioGroupContainsEmptyValue(comp)) {
  ...
  function radioGroupContainsEmptyValue(comp) {
    return Ext.Array.some(comp.getBoxes(), function(box) {
...
//          proxy: Ext.create('gw.ext.ModelProxy', {url:'dummy2'}),
    ...
    setValueWithSuspendChangeEvent: function(compId, newValue) {
      var targetComp = Ext.getCmp(compId);
      if (targetComp && targetComp.setValue) {
        targetComp.suspendCheckChange++;  //suspend change event
        targetComp.setValue(newValue);
        targetComp.suspendCheckChange--;
      }
    },

    linkBottomAndTopToolbar: function(panel) {
      if (panel.bbar && panel.bbar.items) {
        // adding the bottom input id to it so it can reflect the value when it is changed
        Ext.each(panel.tbar.items, function(item) {
          // searching the bottom toolbar to find the component with the event id equal to the top toolbar component
          Ext.each(panel.bbar.items, function(dupItem) {
            if (dupItem.eventId == item.id) {
              item.otherToolbarReflectId = dupItem.id;
              return false;  // found it. break from the loop
            }
          });
        });
        // Link the bottom toolbar items with the top toolbar items. Since all bottom component already have event id
        // matches to the actual widget, so we just need to copy eventId to otherToolbarReflectId
        Ext.each(panel.bbar.items, function(bItem){
          bItem.otherToolbarReflectId = bItem.eventId;
        });
      }
    }
  }
