...
gw.app = function() {
  ...
  /** width of M in pixels for a form field, default to 8 just in case we can't compute */
  var _formFieldMCharWidthInPixels = 8;
  ...
  /**
   * Debug log in case the localization messages associated with communication issues fail.
   * @private
   */
  ...
  function clearGridSelectionIfNavigateToNewLocation(isPostOnChange, params) {
    if (!isPostOnChange && !params.renderViewRootOnly){
        ...
        // In this case we should clear the gridLastSelection in currentPage.
        var gridSelectionIds = Object.keys(gw.app.gridLastSelection);
        var gridSelectionInCurrentPage = gridSelectionIds.filter(function (a) {
            return (Ext.ComponentManager.get(a) != null)
        });
        for (var lvIndex in gridSelectionInCurrentPage) {
            delete gw.app.gridLastSelection[gridSelectionInCurrentPage[lvIndex]];
        }
    }
  ...
  function _callServer(eventSource, eventParam, params, postForm, maskElem, auxConfig) {
    ...
    clearGridSelectionIfNavigateToNewLocation(skipFocusOnPOC, params);
  ...
  function _requestCallback (requestOptions, success, response) {
    ...
    try {
      ...
      gw.app.showAlert(gw.app.localize("ExtJS.Alert.InvalidServerResponse", response.responseText));
      return;
    }
      ...
      Ext.Object.each(requestOptions.scrollPositions, function(id, scrollInfo) {
        ...
        if (comp && (el = comp.getTargetEl()) && el.dom) { // @SenchaUpgrade getTargetEl() is private
          Ext.Object.each(scrollInfo, function(side, value) {
            if (value != 0) {
              el.scrollTo(side, Ext.String.htmlEncode(value), false);
            }
          });
        }
      });
    ...
    if (lvFocusRow){
      ...
      if (grid){
        ...
        if (gridSelModel && (gridSelModel instanceof Ext.selection.CellModel)){
          ...
          var newPosition = {row: _focusDataRecordIndex, column: _focusDataColumnPos};
          gridSelModel.setCurrentPosition(null);
  ...
  function _showFrame(frameId) {
    var popupWindow =
      gw.app.openPopup(_serverURL + '?inFrame=' + frameId + '&r=' + Math.floor(Math.random() * 10000),
        frameId,
  ...
  function _computeWidestCharInPixels() {
    ...
    _formFieldMCharWidthInPixels = dimensions ? dimensions.width : _formFieldMCharWidthInPixels
  }
  ...
  return {
    ...
    onFocusDataRecordIndex : function(dataRecordIndex, dataColumnPos, parentId) {
      // assert to make sure both dataRecordIndex and  dataColumnPos are integer
...
//          {key:Ext.EventObject.ENTER, shift:false, alt:false, fn:_handleEnterKey} // ENTER key
     ...
     */
    handleAction: function(evt, htmlElem, options, comp) {
      if (!htmlElem.disabled) {
        ...
        if (Ext.isString(htmlElem)) {
          ...
          var compEventId = (comp) ? comp.eventId : null;
          eventSource = el.getAttribute('eventId') || compEventId || el.getAttribute('id');
    ...
    handleCompAction : function (comp, evt, el) {
      ...
      function doIt(param) {
        ...
        if (comp.postOnChangeTarget) {
          gw.app.requestViewRoot(gw.app.getPocId(comp, el && el.dom || comp.getEl().dom), {updateData:true, postOnChangeTarget:comp.postOnChangeTarget});
        } else {
          gw.app.handleAction(evt, el && el.dom || comp.getEl().dom, options, comp)
        }
      }
    ...
    getPocId:function(comp, htmlEl) {
      var el = Ext.fly(htmlEl);
      var compEventId = (comp) ? comp.eventId : null;
      var eventId = compEventId || el.getAttribute("eventId");
      ...
      function applyChanges() {
        ...
        if (gw.app.isPostChangeToServer(field)) {
          ...
          if (targetId != null) {
            ...
            gw.app.requestViewRoot(field.eventParam || field.eventId || field.name || field.checkboxName || field.id, params, Ext.fly(targetId),
                (callback) ? {postCallbackScope:field, postCallback:callback, postedToServer:true} : undefined)


          } else {
            _callServer('_refresh_',  field.eventParam || field.eventId || field.name || field.hiddenName || field.id, extraParams, true);
          }
        } else {
          maybeResetFocus();
          if (callback) {
    ...
    widthOfMCharInPixels : function() {
      return _formFieldMCharWidthInPixels;
    },
