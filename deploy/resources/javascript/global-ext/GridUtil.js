gw.GridUtil = function(){
  ...
  return {
    ...
    updateFlaggedButtons: function(flagged, checkedItems, grid) {
      Ext.each(flagged, function(btnId) {
        ...
        if (btn && !btn.initialConfig.disabled) {
          var dupBtn = (btn.otherToolbarReflectId) ? Ext.ComponentManager.get(btn.otherToolbarReflectId) : null;
     ...
     */
    addFiltersToPanel:function (panel, filters, bbarFilters) {
      if (!panel.tbar) {
        panel.tbar = {xtype:'gtoolbar'}
      }
      panel.tbar.items = filters.concat(['-']).concat(panel.tbar.items || []);
      if (bbarFilters) {
        Ext.each(bbarFilters, function(item) {
           item.otherToolbarReflectId = item.eventId;
        });
        if (!panel.bbar) {
          panel.bbar = {xtype: 'gtoolbar'}
        }
        panel.bbar.items = bbarFilters.concat(['-']).concat(panel.bbar.items || []);
      }
    },
