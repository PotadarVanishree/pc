  ...
  return {
    ...
    replace : function (cmdInfo) {
      ...
      if (!cmdInfo.items) {
        ...
        gw.Debug.log("Received 'replace' cmd but more than one item was sent (#items: " + cmdInfo.items.length + ")");
      } else {
    ...
    /**
     * handle selection and focus for either newly added row in a LV or focusOnStartEditing cell.
     * @param cmdInfo The command, containing the row and column to focus on and the container ID
     */
    ...
    updateValues : function(cmdInfo, response) {
      ...
      for (var i = 0; i < cmdInfo.items.length;i++) {
        ...
        if (comp instanceof gw.SimpleGrid && item.items) {
          gw.cmdProc.updateValues(item, response);
        }
      }
    },
    ...
    restoreScroll : function(cmdInfo, response, requestOptions) {
      requestOptions.scrollPositions = Ext.JSON.decode(Ext.String.htmlDecode(cmdInfo.scrollPositions)); // use requestOptions to track "request scope"
    },
    ...
    openSouthPanel : function() {
      Ext.ComponentManager.get("southPanel").expand();
    },
