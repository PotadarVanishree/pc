...
class SubmissionProcess extends NewTermProcess {
  ...
  function canSaveDraft() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Submission.SaveDraft)
  }
  ...
  function canConvertToFullApp() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Submission.ConvertToFullApp)
            .checkEditPermission()
            .checkQuickQuote()
            .checkNoBranchLocked()
    ...
    PCProfilerTag.BIND_FINISH.execute(\ -> {
      ...
      if (Job.BindOption == "BindAndIssue") {
        ...
        PCProfilerTag.BIND_HOLD_ISSUANCE.execute(\ -> {
          ...
          startChecksFor(displaykey.Job.Process.Submission.FinishBinding).checkNoActivePeriods().assertOkay()
  ...
  function finishIssuing() {
    ...
    startChecksFor(displaykey.Job.Process.Submission.FinishIssuing).checkNoActivePeriods().assertOkay()
  ...
  function canDeclineJob() : JobConditions {
    var conditions = startChecksFor(displaykey.Job.Process.Submission.DeclineJob)
  ...
  function canDecline() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Submission.Decline)
            .checkPermission(Permissions.Decline)
            .checkNotPromoted()
  }
  ...
  function canNotTakeJob() : JobConditions {
    var conditions = startChecksFor(displaykey.Job.Process.Submission.NotTakeJob)
  ...
  function canNotTake() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Submission.NotTake)
        .checkPermission(Permissions.NotTaken)
        .checkBranchNotLocked()
        .checkNotPromoted()
  }
