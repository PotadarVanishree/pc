...
class GenericClauseSelectionForm extends FormData implements GenericFormInference {
  ...
  override function populateInferenceData(context : FormInferenceContext, availableStates : Set<Jurisdiction>) {
    var accepted = new ArrayList<Coverable>()
    for (cov in _coverables) {
      var clause = cov.getCoverageConditionOrExclusion(_clausePattern)
      if (atLineLevel(clause)) {
        if (availableStates.intersect(context.getLineCoveredStates(_line)).HasElements) {
          accepted.add(cov)
        }
      } else if (availableStates.contains(context.getCoverableState(cov))) {
        accepted.add(cov)
      }
    }
    _coverables = accepted.toTypedArray()
  }
  ...
  private function atLineLevel(clause : Clause) : boolean {
    return clause.Pattern.OwningEntityType == _line.Pattern.PolicyLineSubtype.Code
  }
