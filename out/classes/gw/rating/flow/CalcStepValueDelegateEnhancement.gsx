...
enhancement CalcStepValueDelegateEnhancement: entity.CalcStepValueDelegate {
  private static function determineConstantTypeForStringValue(constantValue: String): IType {
    var t = deduceTypeFromValue(constantValue)
  ...
  property get TypeOfConstant(): IType {
    ...
    if (typeIsInteger(this.InScopeValueType)) {
      return Integer
    }
  ...
  property get LocalizedConstantValue(): String {
    var valueType = this.InScopeValueType ?: deduceTypeFromValue(this.ConstantValue).Name

    try {
      if (typeIsBigDecimal(valueType)){
      ...
      } else if (typeIsInteger(valueType)){
  ...
  static function deduceTypeFromValue(str : String) : IType {
    ...
    // "true/false" and null are not localizable
    if (str.matches("^(true|True|TRUE|false|False|FALSE)$"))  {
      return java.lang.Boolean
    } else if (str.matches("^(null|Null|NULL)$")){
      return null
    } else if (str.matches("^\".*\"$")) {
      // this will accept some illegal strings, e.g. "a"b" (should be "a\"b")
      // we rely on validation elsewhere to prevent this case, because a correct
      // regular expression would be overkill.
  ...
  property get IsNumeric(): boolean {
   return TypeOfConstant == BigDecimal or TypeOfConstant == Integer // or TypeOfConstant == Number
  }
  ...
  property get OwningStep(): CalcStepDefinition {
    if (this typeis CalcStepDefinitionOperand) {
      return this.CalcStep
    } else // if (this typeis CalcStepDefinitionArgument) {
        return (this as CalcStepDefinitionArgument).Operand.CalcStep
  }
  ...
  static function isCoverageType(valueType : IType) : boolean {
    return entity.Coverage.Type.isAssignableFrom(valueType)
  }

  static function isCovTermType(valueType : IType) : boolean {
    return CovTerm.Type.isAssignableFrom(valueType)
       // we need to check CovTermPattern also, for backward compatibility.
       or CovTermPattern.Type.isAssignableFrom(valueType)
  }

  property get IsCovTerm() : boolean {
    var valueType = TypeSystem.getByRelativeName(this.InScopeValueType)
    return isCovTermType(valueType)
  }
}
