package gw.webservice.pl.pl800.personaldata
...
@WsiWebService("http://guidewire.com/pl/ws/gw/webservice/pl/pl800/personaldata/PersonalDataDestructionAPI")
@WsiExposeEnumAsString(typekey.DestructionRequestStatus)
@Export
class PersonalDataDestructionAPI {
  ...
  function destroyUser(username : String) : boolean {
    ...
    if (user == null) {
      ...
      if (user.Obfuscated and user.Contact.Obfuscated and user.Credential.Obfuscated) {
        ...
        _logger.warn("On destroyUser, an entity returned false from obfuscation: User - " + user.Obfuscated + ", UserContact - " + user.Contact.Obfuscated + ", Credential - " + user.Credential.Obfuscated)
        return false
      }
    }
  }
  ...
  private property get Destroyer() : PersonalDataDestroyer {
    if (gw.plugin.Plugins.isEnabled(PersonalDataDestruction)) {
      return gw.plugin.Plugins.get(PersonalDataDestruction).Destroyer
    }
