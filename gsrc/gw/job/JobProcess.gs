...
abstract class JobProcess implements gw.api.job.IJobProcess {
  ...
  protected function canPromoteToDraft() : JobConditions {
    return startChecksFor(displaykey.Job.Process.BeginEditing)
        .checkBranchNotLocked()
        .checkEditPermission()
        .checkNew()
  }
  ...
  function canStartChangeEditEffectiveDate() : JobConditions {
    return startChecksFor(displaykey.Job.Process.ChangeEditEffectiveDate)
  ...
  function canRequestQuote() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Quote)
  ...
  function canStartCopyPolicyData() : JobConditions {
    return startChecksFor(displaykey.Job.Process.CopyData)
        .check(false, "Unsupported operation")
  }
  ...
  function canMakeNewVersion(): JobConditions {
    var jobConditions = startChecksFor(displaykey.Job.Process.MakeNewVersion)
  ...
  protected function canFinishBind() : JobConditions {
    return canFinishBind(startChecksFor(displaykey.Job.Process.FinishBinding))
  }
   ...
   */
  function review() {
    startChecksFor(displaykey.Job.Process.Review).checkBranchNotLocked().assertOkay()
  ...
  function canHandlePreemptions() : JobConditions {
    return startChecksFor(displaykey.Job.Process.HandlePreemptions)
  ...
  private function checkExpireJob() : JobConditions {
    var conditions = startChecksFor(displaykey.Job.Process.ExpireJob).checkJobNotComplete()
  ...
  function canExpire() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Expire)
  ...
  function canWithdrawJob() : JobConditions {
    var activePeriods = Job.ActivePeriods
    var conditions = startChecksFor(displaykey.Job.Process.WithdrawJob)
        .checkJobNotComplete()
  ...
  function canWithdraw() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Withdraw)
        .checkBranchNotLocked()
  ...
  function withdrawOtherActivePeriods() {
    startChecksFor(displaykey.Job.Process.WithdrawOther).checkTermNotArchived().assertOkay()
