...
class AuditProcess extends JobProcess {
  ...
  override function canEdit() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Audit.Edit)
            .checkEditPermission()
            .checkQuoteIsValid()
            .checkJobNotComplete()
  }
  ...
  function canCreateAuditPackage() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Audit.CreateAuditPackage)
  ...
  function canWaive() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Audit.Waive)
  ...
  function canComplete() : JobConditions {
    var completionConditions = startChecksFor(displaykey.Job.Process.Audit.Complete)
  ...
  function canFinishReversal() : JobConditions {
    return checkJobCompletionConditions(startChecksFor(displaykey.Job.Process.Audit.FinishReversal))
  }
