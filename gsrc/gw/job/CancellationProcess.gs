...
class CancellationProcess extends JobProcess {
  ...
  function canEditCancellationDate() : JobConditions {
    var jobConditions = startChecksFor(displaykey.Job.Process.Cancellation.EditCancellationDate)
  ...
  function canRescheduleCancellation() : JobConditions {
    var jobConditions = startChecksFor(displaykey.Job.Process.Cancellation.RescheduleCancellation)
  ...
  function setCancellationDate(cancellationDate : DateTime) {
    ...
    var withinPeriod = startChecksFor(displaykey.Job.Process.Cancellation.CrossSliceBoundaries)
  ...
  function canFinishSendNotices() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Cancellation.FinishSendingNotices)
            .checkBranchNotLocked()
            .checkQuoteIsValid()
  ...
  function canRescind(ignoreCancelProcessDate : boolean) : JobConditions {
      ...
      return startChecksFor(displaykey.Job.Process.Cancellation.Rescind)
  ...
  function canFinishRescind() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Cancellation.FinishRescind)
            .checkPermission(Permissions.RescindCancellation)
            .checkBranchNotLocked()
            .checkQuoteIsValid()
            .checkCurrentNotificationsSent()
  ...
  function canIssueCancellation() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Cancellation.IssueCancellation)
            .checkBranchNotLocked()
            .checkQuoteIsValid()
            .checkCurrentNotificationsSent()
            .checkNoUnhandledPreemptions()
  ...
  function canEscalate() : JobConditions {
    return startChecksFor(displaykey.Job.Process.Cancellation.JobEscalation)
            .checkBranchNotLocked()
  }
